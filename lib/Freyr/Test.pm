...
=sub test_irc_msg( $irc, $send, $test, $recv )

Send an IRC message over the $irc object and test the routing of the message.

$send is a message to send over IRC using Mojo::IRC's offline testing method. You must
include the full IRC message, like:

    :nick!user@host PRIVMSG #defocus Hello!

$test must be either C<like> or C<unlike>, and L<Test::More>'s L<Test::More/like|like> or 
L<Test::More/unlike> will be used.
